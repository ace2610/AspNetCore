            ...
            app.Map("/refresh_token", signinApp =>
            {
                signinApp.Run(async context =>
                {
                        ...
                        || string.Equals(MicrosoftAccountDefaults.AuthenticationScheme, currentAuthType))
                    {
                        ...
                        authProperties.UpdateTokenValue("access_token", payload.RootElement.TryGetProperty("access_token", out var property) ? property.GetString() : null);
                        refreshToken = payload.RootElement.TryGetProperty("refresh_token", out property) ? property.GetString() : null;
                        if (!string.IsNullOrEmpty(refreshToken))
                        {
                            authProperties.UpdateTokenValue("refresh_token", refreshToken);
                        }
                        if (payload.RootElement.TryGetProperty("expires_in", out property) && property.TryGetInt32(out var seconds))
                        {
